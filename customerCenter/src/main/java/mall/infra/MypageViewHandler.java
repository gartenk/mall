...
public class MypageViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDeliveryConfirmed_then_UPDATE_2(@Payload DeliveryConfirmed deliveryConfirmed) {
        try {
            if (!deliveryConfirmed.validate()) return;
                // view 객체 조회

                List<Mypage> mypageList = mypageRepository.findByOrderId(deliveryConfirmed.getOrderId());
                for(Mypage mypage : mypageList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
